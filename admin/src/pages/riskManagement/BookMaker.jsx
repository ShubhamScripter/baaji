...
const extractBookmakerMarket = (bettingData) => {
  if (!Array.isArray(bettingData)) return null;
  const market = bettingData.find(
    (item) =>
      item?.mname === "Bookmaker" || item?.mname === "Bookmaker IPL CUP"
  );
...
const OddsDetails = ({ bettingData, isLoading, fallbackOdds }) => {
  ...
  const market = extractBookmakerMarket(bettingData);
...
function BookMaker() {
  ...
  return (
    <div className="mt-6 rounded-[5px]">
      ...
      {showMasterDownline && (
        <div className="fixed top-0 left-0 w-full h-full z-9999 bg-black/40 flex items-start justify-center">
          <motion.div
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
            transition={{ duration: 0.4 }}
            className="h-full w-full rounded-lg bg-[#fbf9ed] shadow-lg"
          >
