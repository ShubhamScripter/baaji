...
function BettingProfitLoss() {
  ...
  const fetchProfitLossData = async (customStartDate = null, customEndDate = null, customStartTime = null, customEndTime = null) => {
    try {
      setLoading(true);
      ...
      const response = await axiosInstance.get(`/get/profit-loss-by-downline-reports-userData-v2?${queryParams}`);
  ...
  const fetchCasinoData = async (customStartDate = null, customEndDate = null, customStartTime = null, customEndTime = null) => {
    try {
      setLoading(true);
      const useStartDate = customStartDate || startDate;
      const useEndDate = customEndDate || endDate;
              ...
              <div className="flex justify-between items-center gap-2 mt-3">
                ...
                <div className="flex justify-center items-center gap-2">
                  ...
                  <input
                    ...
                    className="border border-[#aaa] shadow-[inset_0_2px_0_0_#0000001a] bg-[#fff] min-w-[170px] text-xs p-2"
                  />
                  <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="border border-[#aaa] shadow-[inset_0_2px_0_0_#0000001a] text-xs p-2" />
                </div>
                <div className="flex justify-center items-center gap-2">
                  ...
                  <input
                    ...
                    className="border border-[#aaa] shadow-[inset_0_2px_0_0_#0000001a] bg-[#fff] min-w-[170px] text-xs p-2"
                  />
                  <input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className="border border-[#aaa] shadow-[inset_0_2px_0_0_#0000001a] text-xs p-2" />
                </div>
              </div>
              <div className="mt-4 flex gap-2 pb-2">
                <button className="border border-[#bbb] text-xs font-[700] bg-[linear-gradient(180deg,_#fff,_#eee)] p-2 rounded-sm cursor-pointer" onClick={() => {
                  const today = new Date().toISOString().split("T")[0];
                  setStartDate(today); setEndDate(today); setStartTime("00:00"); setEndTime("23:59");
                ...
                <button className="border border-[#bbb] text-xs font-[700] bg-[linear-gradient(180deg,_#fff,_#eee)] p-2 rounded-sm cursor-pointer" onClick={() => {
                  ...
                  setStartDate(y); setEndDate(y); setStartTime("00:00"); setEndTime("23:59");
                  if (selectedType === "Casino") fetchCasinoData(y, y, "00:00", "23:59");
                  else fetchProfitLossData(y, y, "00:00", "23:59");
                }}>
                  From Yesterday
                </button>
                <button className="border border-[#cb8009] text-xs font-[700] bg-[#ffcc2f] p-2 rounded-sm hover:bg-[#ffa00c] cursor-pointer" onClick={() => {
                ...
                <button className="border border-[#cb8009] text-xs font-[700] bg-[#ffcc2f] p-2 rounded-sm hover:bg-[#ffa00c] cursor-pointer" onClick={() => {
                  ...
                  setStartDate(d.startDate); setEndDate(d.endDate); setStartTime("00:00"); setEndTime("23:59");
                  if (selectedType === "Casino") fetchCasinoData(d.startDate, d.endDate, "00:00", "23:59");
                  else fetchProfitLossData(d.startDate, d.endDate, "00:00", "23:59");
                }}>
                  Reset
                </button>
              </div>
