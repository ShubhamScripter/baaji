...
function collectEsidMatches(normalized) {
  return [
    ...(normalized.data?.t1 || []),
    ...(normalized.data?.t2 || []),
  ];
...
function findMatchByAnyId(matches, gameId) {
  const needle = String(gameId);
  return matches.find((m) =>
    [m.gmid, m.oldgmid, m.beventId].some(
      (id) => id != null && String(id) === needle
    )
  );
}

function resolvePriveteGmid(match) {
  return match?.gmid ?? match?.oldgmid ?? null;
}
...
export function createProviderC() {
  ...
  /** beventId/oldgmid from UI → API gmid for /getPriveteData */
  const gmidResolveCache = new Map();
  const GMID_CACHE_MS = 5 * 60 * 1000;

  async function resolveGmidForPriveteData(gameId, apiSid) {
    const cacheKey = `${apiSid}:${gameId}`;
    const cached = gmidResolveCache.get(cacheKey);
    if (cached && Date.now() - cached.ts < GMID_CACHE_MS) {
      return cached.gmid;
    }

    const listRes = await axios.get(`${API_URL}/esid`, {
      params: { sid: apiSid, key: API_KEY },
    });
    const normalized = normalizeEsidResponse(listRes.data);
    if (!normalized.success) return null;

    const matched = findMatchByAnyId(collectEsidMatches(normalized), gameId);
    const resolved = resolvePriveteGmid(matched);
    if (resolved == null) return null;

    gmidResolveCache.set(cacheKey, { gmid: resolved, ts: Date.now() });
    return resolved;
  }
  ...
  return {
    ...
    async fetchMatchData(gameId, sportId) {
      ...
      if (cached && Date.now() - cached.ts < GMID_CACHE_MS) {
        candidates.push(cached.gmid);
      }
      candidates.push(gameId);

      const tried = new Set();
      for (const gmid of candidates) {
        const key = String(gmid);
        if (tried.has(key)) continue;
        tried.add(key);

        try {
          const response = await axios.get(`${API_URL}/getPriveteData`, {
            params: { key: API_KEY, gmid, sid: apiSid },
          });
          if (response.data?.success) {
            gmidResolveCache.set(cacheKey, { gmid, ts: Date.now() });
          }
      ...
      const resolved = await resolveGmidForPriveteData(gameId, apiSid);
      if (resolved == null || tried.has(String(resolved))) {
        const notFound = new Error(`Match ${gameId} not found on Provider C`);
        notFound.response = { status: 404 };
        throw notFound;
      }

      const response = await axios.get(`${API_URL}/getPriveteData`, {
        params: { key: API_KEY, gmid: resolved, sid: apiSid },
      });
      return response.data;
    },
